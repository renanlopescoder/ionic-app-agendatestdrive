angular.module('starter').controller('AgendamentoController', function($scope, DatabaseValues, CarroService, $ionicPopup, $state){
  $scope.agendamentos = [];
  ...
  $scope.reenviar = function(agendamento){
    ...
    CarroService.salvarPedido(agendamentoFinalizado).then(
      function(dados){
        DatabaseValues.setup();
        DatabaseValues.bancoDeDados.transaction(function(transacao){
      ...
      function (erro){
                $ionicPopup.alert({
          title : "Erro",
          template : "Servidor continua com erro favor tentar mais tarde",
        })
      }
    );
  }
